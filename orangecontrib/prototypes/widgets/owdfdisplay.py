...
from orangecontrib.prototypes.widgets.utils.textimport import StampIconEngine
...
class DataDelegate(QStyledItemDelegate):
    def __init__(self, *args, **kwargs):
        ...
        @lru_cache(maxsize=50 * 200)
        def sttext(text: str) -> QStaticText:
            return QStaticText(text)
        self.__static_text_cache = sttext
        self.__static_text_lru_cache = LRUCache(4000)

    def displayText(
            self, value, locale,
            # This is the worst runtime ever. Of all time.
            __isinstance=isinstance,
            __Real=numbers.Real,
            __Integral=numbers.Integral,
            __String=(str, np.str_),
            __isnan=math.isnan
    ):
        if __isinstance(value, __Integral):
            return super().displayText(int(value), locale)
        elif __isinstance(value, __Real):
            if __isnan(value):
                return "N/A"
            else:
                super().displayText(float(value), locale)
        elif __isinstance(value, __String):
            return str(value)
